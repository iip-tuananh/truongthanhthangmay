        ...
        view()->composer('*', function ($view) 
        {
            ...
            $blogCate = BlogCategory::with([
                'listBlog' => function ($query){
                    $query->select('id','slug','title','image','category')->take(2);
                }
            ])
            ->where('status',1)
            ->orderBy('id','DESC')
            ->get(['id','name','slug','avatar']);
            $listblogcate = BlogCategory::where('status',1)->get()->map(function ($query){
                $query->setRelation('listBlog',$query->listBlog->where('status',1)->take(3));
                return $query;
            } );
